  ...
  function initFlipIconAnimation() {
    ...
    let tl_flip_to_process = gsap.timeline({
      scrollTrigger: {
        ...
        start: "top 100%",
        end: "bottom 60%", // Adjust to control when first animation completes
        scrub: 0.5,
        ...
        onLeaveBack: () => {
          // When scrolling back up past the trigger start point
          if (sj_banner_1_icon_group_1.parentElement === sj_target_container_icon_group_process_section) {
            // Go back to original parent
            const originalParent = originalElements.get(sj_banner_1_icon_group_1);
            if (originalParent) {
              originalParent.appendChild(sj_banner_1_icon_group_1);
              // The timeline will handle the animation in reverse
            }
          }
        }
      }
    ...
    let tl_flip_to_contact = gsap.timeline({
      scrollTrigger: {
        ...
        start: "top 60%", // Start after we're fully in the first position
        end: "bottom 100%",
        scrub: 0.5,
        // markers: true, // Uncomment for debugging
        onEnter: () => {
