...
function small_joys_home() {
  ...
  function killAll() {
    ...
    // Reset DOM: Move the element back to its original container
    const movedElement = document.querySelector("#w-node-_0b70cb18-2388-89ef-fff0-75b7f977beda-c738a86b");
    const originalContainer = document.querySelector(".sj_banner_1_icon_group_container");
    
    // Only move it back if it's not already in the original container
    if (movedElement && originalContainer && movedElement.parentElement !== originalContainer) {
