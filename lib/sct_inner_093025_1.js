/** sct_inner js 
 * working on npm run dev but not when it's deployed.
*/
...
function sct_inner() {
  ...
  function init() {
    console.log('sct_inner.js init works');
  ...
  function cs_intro_anim() {
    ...
    // Initial visibility
    let tl_sct_inner = gsap.from("html", { duration: 0, autoAlpha: 0});
    tl_sct_inner.play();
    ...
    let sct_inner_banner = document.querySelector(".sct_grid_1.inner_banner");
    ...
    let sct_inner_banner_subheader_text = document.querySelector(".sct_inner_banner_subheader_text");
    
    // Check if elements exist before proceeding
    // if (!ux_cs_banner_image_wrapper || !ux_cs_banner_image || !sct_inner_banner || 
    //     !sct_inner_banner_header_text || !sct_inner_banner_subheader_text) {
    //   console.warn("Required elements for animation not found, aborting");
    //   return;
    // }
    
    // Store original content for reset
    originalContent.header = sct_inner_banner_header_text ? sct_inner_banner_header_text.innerHTML : '';
    originalContent.subheader = sct_inner_banner_subheader_text ? sct_inner_banner_subheader_text.innerHTML : '';
    originalContent.trail = [];
    sct_inner_banner_breadcrumbs_text.forEach((trail, i) => {
    ...
    let splitLines_header = new SplitText(sct_inner_banner_header_text, {
      type: "lines", 
    ...
    let splitLines_subheader = new SplitText(sct_inner_banner_subheader_text, {
      type: "lines", 
      linesClass: "split-line",
      lineThreshold: 0.5
    });
    ...
    sct_inner_banner_breadcrumbs_text.forEach(trail => {
...
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.body.classList.contains("sct_inner")) {
      window.uxCaseStudyModule = sct_inner();
    }
  });
} else {
  // DOM already loaded
