...
function example_16() {
  ...
  function init() {
    ...
    // Set all cards to display:flex but hide them with autoAlpha
    gsap.set(sc_cards, { display: "flex", autoAlpha: 0 });
    // Make only the first card visible
    ...
    sc_cards.forEach((card, index) => {
        ...
        .fromTo(cards_index, 
            {
                ...
                display: "flex" // Ensure the card is displayed as flex
            },
            {
                rotationX: 0,
                rotationY: 0,
                ...
                transformOrigin: "50% 100% 0",
                autoAlpha: 1,
                display: "flex", // Maintain flex display during animation
                duration: 2,
        ...
        // First fade out the card a bit before the flip animation
        .to(cards_index, {
            autoAlpha: isMobile ? 0 : 0.7, // On mobile, hide immediately for cleaner transition
            duration: 0.5,
            ease: "power1.in"
        }, 9.8)
        ...
        .to([headerLines, subheaderLines], {
            autoAlpha: isMobile ? 0 : 1, // Only on mobile
