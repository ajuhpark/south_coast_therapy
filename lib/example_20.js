      ...
      Example.timescale = function() {
          ...
          var explosion = function(engine) {
              ...
              for (var i = 0; i < bodies.length; i++) {
                  ...
                  if (!body.isStatic && body.position.y >= matter_container_2.clientHeight * 0.75) {
          ...
          var colors = [
            ...
            // getCSSVar('--_theme---color_group_2--color-300'),
            // getCSSVar('--_theme---color_group_3--color-100'),
            // getCSSVar('--_theme---accent_color_1_light'),
            getCSSVar('--_theme---color_group_2--color-200'),
            // getCSSVar('--color--purple--300')


            getCSSVar('--_theme---color_group_2--color-200'),
            // getCSSVar('--sct_azure'),
            getCSSVar('--sct_sky_blue'),
            getCSSVar('--sct_sunglow'),
            getCSSVar('--sct_papaya_whip'),
            // getCSSVar('--sct_lapis_lazuli')
    ...
    function matter_container_3(){
      ...
      Example.timescale = function() {
          ...
          var bodyOptions = {
              ...
              // frictionAir: 0.01,       // Add air resistance to slow down items
              // friction: 0.0001,     // Very low surface friction
              friction: 0.001,       // Reduced surface friction for smoother movement
              restitution: 0.80     // Higher bounciness to maintain energy (0 = no bounce, 1 = perfect bounce)
          };
          ...
          World.add(world, Composites.stack(
            ...
            20,                                          // columnGap (horizontal spacing between shapes)
            120,                                         // rowGap (vertical spacing between shapes)
            function(x, y) {
              // Circle radius relative to container size (2-8% of container width) - with fresh randomness
          ...
          var explosion = function(engine) {
              ...
              for (var i = 0; i < bodies.length; i++) {
                  ...
                  // Only affect non-static bodies that are near the bottom
                  if (!body.isStatic && body.position.y >= matter_container_3.clientHeight * 0.75) {
                      // Calculate force magnitude based on body mass - smaller than matter_container_2
                      var forceMagnitude = 0.005 * body.mass; // Reduced from 0.03 for smaller explosion
          ...
          Events.on(engine, 'afterUpdate', function(event) {
              ...
              // Every 6 seconds (60 FPS * 6 = 360 frames) - longer interval than matter_container_2
              if (counter >= 60 * 8) {
                  // Toggle between normal speed and slow motion
                  if (timeScaleTarget < 1) {
                      timeScaleTarget = 1;      // Switch to normal speed
                  } else {
                      timeScaleTarget = 0.4;   // Switch to slow motion (less extreme than matter_container_2)
                  }
    ...
    function matter_container_4(){
      ...
      Example.timescale = function() {
          ...
          var bodyOptions = {
              ...
              // frictionAir: 0.01,       // Add air resistance to slow down items
              // friction: 0.0001,     // Very low surface friction
              friction: 0.1,       // Reduced surface friction for smoother movement
              restitution: 0.8     // Higher bounciness to maintain energy (0 = no bounce, 1 = perfect bounce)
          };

          // Create a grid of small bouncy circles - spawned from center of container
          World.add(world, Composites.stack(
            matter_container_4.clientWidth / 2 - 50, // x position (starting X coordinate)
            matter_container_4.clientHeight / 2 - 250, // y position (starting Y coordinate - center)
            1,                                         // columns (number of shapes horizontally)
            2,                                          // rows (number of shapes vertically)
            80,                                          // columnGap (horizontal spacing between shapes)
            100,                                         // rowGap (vertical spacing between shapes)
            function(x, y) {
              // Circle radius relative to container size (2-8% of container width) - with fresh randomness
              var minRadius = matter_container_4.clientWidth * 0.12;
              var maxRadius = matter_container_4.clientWidth * 0.15;
              var randomRadius = minRadius + (Math.random() * (maxRadius - minRadius));
              var randomColor = colors[Math.floor(Math.random() * colors.length)];
              var circleOptions = Object.assign({}, bodyOptions, {
                  render: { fillStyle: randomColor }
          ...
          World.add(world, Composites.stack(
              ...
              2,                                         // columns (number of shapes horizontally)
              2,                                         // rows (number of shapes vertically)
              100,                                         // columnGap (horizontal spacing between shapes)
              80,                                         // rowGap (vertical spacing between shapes)
              function(x, y) {
              // Create options with random color for each shape using fresh Math.random
              var randomColor = colors[Math.floor(Math.random() * colors.length)];
              var shapeOptions = Object.assign({}, bodyOptions, {
              ...
              case 0:
                  ...
                  if (Math.random() < 0.8) {
                      // Square rectangles: 3-12% of container width/height
                      var minSize = matter_container_4.clientWidth * 0.05;
                      var maxSize = matter_container_4.clientWidth * 0.1;
                      var randomWidth = minSize + (Math.random() * (maxSize - minSize));
                      var randomHeight = minSize + (Math.random() * (maxSize - minSize));
                      return Bodies.rectangle(x, y, randomWidth, randomHeight, shapeOptions);
                  } else {
                      ...
                      var maxWidth = matter_container_4.clientWidth * .18;
                      var minHeight = matter_container_4.clientHeight * 0.08;
                      var maxHeight = matter_container_4.clientHeight * 0.18;
                      var randomWidth = minWidth + (Math.random() * (maxWidth - minWidth));
                      var randomHeight = minHeight + (Math.random() * (maxHeight - minHeight));
                      return Bodies.rectangle(x, y, randomWidth, randomHeight, shapeOptions);
                  }
              case 1:
                  // Create random polygon with 4-8 sides, radius 2-8% of container width
                  var minRadius = matter_container_4.clientWidth * 0.05;
                  var maxRadius = matter_container_4.clientWidth * 0.08;
                  var randomSides = Math.floor(Math.random() * 5) + 4; // 4-8 sides
                  var randomRadius = minRadius + (Math.random() * (maxRadius - minRadius));
                  return Bodies.polygon(x, y, randomSides, randomRadius, shapeOptions);
              }
          ...
          var explosion = function(engine) {
              ...
              for (var i = 0; i < bodies.length; i++) {
                  ...
                  // Only affect non-static bodies that are near the bottom
                  if (!body.isStatic && body.position.y >= matter_container_4.clientHeight * 0.7) {
                      // Calculate force magnitude based on body mass - smallest force magnitude
                      var forceMagnitude = 0.005 * body.mass; // Even smaller than matter_container_3
          ...
          Events.on(engine, 'afterUpdate', function(event) {
              ...
              // Every 10 seconds (60 FPS * 10 = 600 frames) - longest interval
              if (counter >= 60 * 10) {
                  // Toggle between normal speed and slow motion
                  if (timeScaleTarget < 1) {
                      timeScaleTarget = 1;      // Switch to normal speed
                  } else {
                      timeScaleTarget = 0.5;   // Switch to slow motion (least extreme)
                  }
