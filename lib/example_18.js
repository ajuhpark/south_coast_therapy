...
function example_18() {
  ...
  function init() {
    ...
    // .map() - Loop through each heading element. 
    // so Once text is split into individual characters, 
    // you can animate each character separately:
    ...
    /**
    So pg_splitHeadings becomes an array like this:
    pg_splitHeadings = [
      SplitText_instance_0,  // "Scroll Down"
      SplitText_instance_1,  // "Animated with GSAP" 
      SplitText_instance_2,  // "Greensock"
      SplitText_instance_3,  // "Animation Platform"
      SplitText_instance_4   // "Animation Platform"
    ]
     
    Why We Need [next]
    The next variable tells us which section we're transitioning to:

    If next = 0, we're going to the first section
    If next = 2, we're going to the third section

    The [next] ensures we animate the correct heading for the section we're transitioning to:

    Going to section 0? Animate pg_splitHeadings[0].chars ("Scroll Down")
    Going to section 2? Animate pg_splitHeadings[2].chars ("Greensock")
    Going to section 4? Animate pg_splitHeadings[4].chars ("Animation Platform")

    It's like having 5 different text animations and saying "play animation #3" when transitioning to section 3!
     */


    // ==================== HEADING REVEAL ANIMATION ====================
    // Creates a staggered character animation for section headings
    // we're doing 'return' because the function is called in both the slideIn() and slideOut() functions
    ...
    function slideIn() {
      ...
      const tl = gsap
        ...
        // Trigger the heading text animation. This is why we used 'return' - to call it here.
