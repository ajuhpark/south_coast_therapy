...
function sj_fixed_icon_st() {
  ...
  // Reset all animations to their original positions
  function resetAllAnimationsToOriginal() {
    // Reset our specific element
    let sj_banner_1_icon_group_container_fixed_icons = document.querySelector('.sj_banner_1_icon_group_container_fixed_icons');
    if (sj_banner_1_icon_group_container_fixed_icons) {
      gsap.set(sj_banner_1_icon_group_container_fixed_icons, { clearProps: "all" });
    }
    
    // Reset the logo nav animation elements
  ...
  function reset_sj_fixed_icon_st() {
    ...
    setTimeout(() => {
      ...
      // Force refresh of all ScrollTriggers without killing them
      ScrollTrigger.refresh();
      
      // Dispatch a custom event to notify other modules they should reinitialize
      const resetEvent = new CustomEvent('sj_animation_reset');
  ...
  // Handle resize - reset our specific animation and trigger reset of others
  window.addEventListener("resize", debounce(function(e) {
    console.log("end of resizing - resetting animations");
