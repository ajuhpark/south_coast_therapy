...
function sct_gradient() {
  ...
  function init() {
    ...
    const gradients = document.querySelectorAll('.sct_card_gradient_block__gradient');
    console.log('Found gradients:', gradients.length, gradients);

    // Store all gradient timelines globally for Puppeteer recording
    if (typeof window !== 'undefined') {
      window.sct_gradient_timelines = [];
    }
    ...
    // Expose gradient timeline globally for Puppeteer recording
    if (typeof window !== 'undefined') {
      window.sct_gradient_timelines.push(gradientTimeline);
    }
