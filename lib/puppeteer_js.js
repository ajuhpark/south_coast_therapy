...
const CONFIG = {
  ...
  // Set to full viewport to capture everything
  clip: {
    x: 0,        // X offset from left
    y: 0,        // Y offset from top
    width: 1440,  // Recording width (full viewport)
    height: 900   // Recording height (full viewport)
  },
...
async function recordFrameByFrame(page) {
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    // Seek to specific time in timeline (including gradients)
    await page.evaluate((time) => {
      if (window.sct_banner_tl) {
        window.sct_banner_tl.pause();
        window.sct_banner_tl.seek(time);
      }
      // Also control gradient timelines if they exist
      if (window.sct_gradient_timelines && window.sct_gradient_timelines.length > 0) {
        window.sct_gradient_timelines.forEach(tl => {
          tl.pause();
          tl.seek(time);
        });
      }
