...
function small_joys_home() {
  ...
  // Media query for desktop screens (768px and up)
  const desktopMediaQuery = window.matchMedia('(min-width: 768px)');

  function init() {
    console.log("small_joys_home init function is working");
    
    // Always run the initial grid animation regardless of screen size
    initGridAnimation();
    
    // Only run other animations on desktop screens (768px and up)
  ...
  function handleMediaQueryChange(e) {
    if (e.matches) {
      // Screen is now 768px or wider - initialize animations
      console.log("Screen is now desktop size - initializing animations");
      initDevTools();
      initLogoToNavAnimation();
      ImageParallax();
    } else {
      // Screen is now under 768px - clean up animations
      console.log("Screen is now mobile size - cleaning up animations");
      cleanupDesktopAnimations();
    }
  }

  function cleanupDesktopAnimations() {
    ...
    if (scrollTriggers.bladeImageParallax) {
      scrollTriggers.bladeImageParallax.kill();
      scrollTriggers.bladeImageParallax = null;
    }

    // Reset any transforms that might have been applied
    gsap.set(".sj_banner_1_header_text", { clearProps: "all" });
    gsap.set(".sj_banner_1_header_text_wrapper", { clearProps: "all" });
    gsap.set(".sj_banner_1_image", { clearProps: "all" });
    gsap.set(".sjs_train_1_blade_image", { clearProps: "all" });
  }
  ...
  function initGridAnimation() {
    // This animation runs on all screen sizes
    let tl_ap_grid = gsap.from("html", { duration: 0.3, autoAlpha: 0});
    tl_ap_grid.play();
...
//       fastScrollEnd: true
  ...
  // Listen for media query changes (when user resizes window)
