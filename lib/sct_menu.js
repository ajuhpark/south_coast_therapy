...
function sct_menu() {
  ...
  function init() {
    ...
    function sct_language_change() {
      // Add notranslate class to language button text
      const languageLinks = document.querySelectorAll('.sct_banner_language_link');
      languageLinks.forEach(link => {
        link.classList.add('notranslate');
      });

      // Add CSS to hide Google Translate banner and fix body positioning
      ...
      // Helper function to restore original (close translation)
      function restoreOriginal() {
        // Remove the Google Translate cookies
      ...
      const englishBtns = document.querySelectorAll('[id="sct_navbar_dropdown_english"]');
      const spanishBtns = document.querySelectorAll('[id="sct_navbar_dropdown_spanish"]');
    ...
    function navbar_blur(){

      console.log('sct_navbar_blur init function is working')
      
      // navbar animation
      
      // Get your navbar element
      const navbar = document.getElementById('sct_navbar_full');
      ...
      let navbarTween = gsap.fromTo(navbar, 
        {
          autoAlpha: 1,
          onStart: function() {
        ...
        },
        {
          ...
          onUpdate: function() {
            ...
            const bgColor = `color-mix(in srgb, var(--sct_azure) ${bgOpacity}%, transparent)`;
      ...
      const navbarScrollTrigger = ScrollTrigger.create({
        trigger: "body",
        start: "top top-=100", // Start when body is 100px above viewport top
        end: "top top", // End when body reaches top of viewport
        animation: navbarTween,
        scrub: true,
        invalidateOnRefresh: true, // Recalculate on refresh
        // markers: true
      });

      // Refresh ScrollTrigger on orientation change and resize (for mobile)
      let resizeTimer;
      const handleResize = () => {
    ...
    function mobile_menu_overlay(){
      ...
      // Set initial state for mobile dropdowns - CLOSED
      ...
      const dropdownRows = document.querySelectorAll('.sct_mobile_menu_dropdown_row');
      dropdownRows.forEach(dropdownRow => {
        const newDropdownRow = dropdownRow.cloneNode(true);
        dropdownRow.parentNode.replaceChild(newDropdownRow, dropdownRow);
      });

      // Add fresh event listeners
      const newDropdownRows = document.querySelectorAll('.sct_mobile_menu_dropdown_row');
      newDropdownRows.forEach(dropdownRow => {
        dropdownRow.addEventListener('click', function() {
          const container = this.parentElement;
          const dropdownList = container.querySelector('.sct_mobile_menu_dropdown_list');

          if (dropdownList) {
            const currentHeight = dropdownList.offsetHeight;
    ...
    function initLogoToNavAnimation() {
      ...
      gsap.set(headerText, { willChange: "font-size, line-height" });
      gsap.set(sct_large_logo_drawing_container, { willChange: "height" });
      gsap.set(headerTextWrapper, { willChange: "row-gap" });
