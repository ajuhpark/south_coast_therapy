/** 
 * SCT Menu
 * Rejoice Menu - https://rejouice.com/
 * From sj file.
 */

// import { gsap } from 'gsap'
// gsap.registerPlugin(SplitText)
// gsap.registerPlugin(GSDevTools)


const rejoice_tl = gsap.timeline({ paused: true });

function sct_menu() {
  console.log('sj_menu.js is working')

  function init() {

    console.log('sj_menu init function is working')
    

    // Remove existing event listeners to prevent duplicates
    const openBtn = document.querySelector(".sj_menu_menu-open-btn_2");
    const closeBtn = document.querySelector(".sj_menu_menu-close-btn_2");
    
    if (openBtn) {
      openBtn.replaceWith(openBtn.cloneNode(true));
    }
    if (closeBtn) {
      closeBtn.replaceWith(closeBtn.cloneNode(true));
    }

    // Reset timeline and clear any existing animations
    rejoice_tl.clear();
    rejoice_tl.pause();
    
    // Reset menu elements to initial state
    gsap.set(".sj_menu_menu-overlay", {
      clipPath: "polygon(0 0, 100% 0, 100% 0, 0 0)",
      clearProps: "pointerEvents"
    });
    gsap.set(".sj_menu_menu-link, .sj_menu_btn", {
      opacity: 1,
      y: 0,
      clearProps: "all"
    });
    gsap.set(".sj_menu_video-preview", {
      height: "1px"
    });

    // gsap.from("html", {duration: 0, autoAlpha:0})

    rejoice_tl
      .to(".sj_menu_menu-overlay", {
        duration: 1,
        clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)",
        ease: "power2.out",
      })
      .from(
        ".sj_menu_menu-link, .sj_menu_btn",
        {
          opacity: 0,
          y: 60,
          stagger: 0.05,
          duration: 0.75,
          ease: "power1.inOut",
        },
        "<",
      )
      // .from(
      //   "sj_banner_1_icon_group_container.menu_overlay",
      //   {
      //     opacity: 0,
      //     y: 100,
      //     duration: 1
      //   },
      //   "<"
      // // )
      // .from(".sj_menu_video-preview",
      //   {
      //     height: "1px",
      //   },
      //   "<")
      .to(
        ".sj_menu_video-preview",
        {
          duration: 1,
          height: "200px",
          ease: "power2.out",
          // opacity: 100
        },
        "<"
        // "<=+10",
      )
      // .to(
      //   ".sj_menu_menu-divider",
      //   {
      //     duration: 2,
      //     width: "100%",
      //     ease: "power4.out",
      //   },
      //   "<",
      // )
  
    function openMenu() {
      document.querySelector(".sj_menu_menu-overlay").style.pointerEvents = "all";
      rejoice_tl.play();
    }
  
    function closeMenu() {
      document.querySelector(".sj_menu_menu-overlay").style.pointerEvents = "none";
      rejoice_tl.reverse();
    }
  
    document.querySelector(".sj_menu_menu-open-btn_2").addEventListener("click", openMenu);
    document
      .querySelector(".sj_menu_menu-close-btn_2")
      .addEventListener("click", closeMenu);
    
    // Close menu when clicking outside
    document.addEventListener("click", function(event) {
      const menu = document.querySelector("#sj_sticky_nav");
      const menuOverlay = document.querySelector(".sj_menu_menu-overlay");
      
      // Check if menu is open and click is outside the menu
      if (menuOverlay && menuOverlay.style.pointerEvents === "all" && 
          menu && !menu.contains(event.target)) {
        closeMenu();
      }
    });
    
    rejoice_tl.reverse();
  
    
    // GSDevTools.create({})


    // dropdown link variables
    const sct_banner_1_header_text_wrapper__dropdown = document.querySelector(".sct_banner_1_header_text_wrapper.dropdown");
    const sct_banner_1_dropdown_list = document.querySelector(".sct_banner_1_dropdown_list");
    const sct_banner_1_dropdown_list_row = document.querySelectorAll(".sct_banner_1_dropdown_list_row");
    const sct_banner_1_dropdown_list_border_div = document.querySelectorAll(".sct_banner_1_dropdown_list_border_div");


    const sct_dropdown_tl = gsap.timeline(
      { paused: true }
    );

    // Set initial state for dropdown
    // if (sct_banner_1_dropdown_list) {
    //   gsap.set(sct_banner_1_dropdown_list, {
    //     opacity: 0,
    //     visibility: "visible",
    //     // display: "none"
    //   });
    // }


    // Dropdown hover functionality
    if (sct_banner_1_header_text_wrapper__dropdown && sct_banner_1_dropdown_list) {
      
      // Set proper initial state - ADD THIS
      gsap.set(sct_banner_1_dropdown_list, {
        clipPath: "polygon(0 0, 100% 0, 100% 0, 0 0)"
      });
      gsap.set(sct_banner_1_dropdown_list_row, {
        y: 20,
        opacity: 0
      });

      sct_banner_1_header_text_wrapper__dropdown.addEventListener("mouseenter", function() {
        // Clear and reset timeline
        sct_dropdown_tl.clear();
        
        // Remove the pause here - it might be causing issues
        // sct_dropdown_tl.pause();

        // Reset to initial state (keep this)
        gsap.set(sct_banner_1_dropdown_list, {
          clipPath: "polygon(0 0, 100% 0, 100% 0, 0 0)"
        });
        gsap.set(sct_banner_1_dropdown_list_border_div, {
          height: "0%"
        });
        gsap.set(sct_banner_1_dropdown_list_row, {
          y: 20,
          opacity: 0
        });

        sct_dropdown_tl
          .to(sct_banner_1_dropdown_list, {
            duration: .6,
            clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)",
            ease: "power2.out",
          })
          .to(sct_banner_1_dropdown_list_border_div, {
            duration: .6,
            // clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)",
            height: '100%',
            ease: "power2.out",
          }, "<"
        )
          .fromTo(sct_banner_1_dropdown_list_row,
            {
              y: 20,
              opacity: 0
            },
            {
              y: 0,
              opacity: 1,
              stagger: 0.05,
              duration: 0.45,
              ease: "power1.inOut",
            },
            "<",
          );

        // Play the timeline
        sct_dropdown_tl.play();
      });

      sct_banner_1_header_text_wrapper__dropdown.addEventListener("mouseleave", function() {
        sct_dropdown_tl.reverse();
        // sct_dropdown_tl.eventCallback("onReverseComplete", function() {
        //   gsap.set(sct_banner_1_dropdown_list, { display: "none" });
        // });
      });
    }

    
  }

  // Initialize immediately and also on load
  init();
  
  window.addEventListener("load", function(event) { 
    init(); 
  });
}

if (document.body.classList.contains('sj_menu')) {
  sj_menu();
}

export default sct_menu