...
function sct_home() {
  ...
  function init() {
    ...
    sct_banner_tl
      ...
      // Stats Counter
      let sct_stats_section = document.querySelector("#sct_stats_section")

      if (sct_stats_section) {
        const counterConfigs = [
          { id: '#sct_stat_text_1', targetValue: 100 },
          { id: '#sct_stat_text_2', targetValue: 75 },
          { id: '#sct_stat_text_3', targetValue: 50 }
        ]

        // Create timeline first
        let tl_counter = gsap.timeline();

        // Add counter animations
        counterConfigs.forEach(config => {
          let counter = { value: 0 }

          tl_counter.to(counter, {
            value: config.targetValue,
            duration: 1,
            ease: "power1.out",
            onUpdate: function() {
              let progress = Math.round(counter.value)
              $(config.id).text(progress)
            }
          }, 0)
        })
        ...
        ScrollTrigger.create({
          trigger: sct_stats_section,
          start: "top bottom",
          // markers: true,
          animation: tl_counter
        })
      }
